...
void SceneNode::Update(glm::mat4 projectionMatrix, Camera* camera){
								if(object!=nullptr) {
																...
																if (parent!=nullptr) { // Child Node (Not Root Node)
																								this->getWorldTransform().loadIdentity();
																								this->getWorldTransform().applyTransform(parent->getWorldTransform());
																								this->getWorldTransform() = this->getWorldTransform() * this->getLocalTransform();

																}
																else { // Root Node (Sun)
																								this->getWorldTransform().loadIdentity();
																								//this->getWorldTransform().getInternalMatrix() = projectionMatrix;
																								this->getWorldTransform() = this->getWorldTransform() * this->getLocalTransform();
																}
